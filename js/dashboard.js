...
// ── ANALISI ECONOMICA ────────────────────────────────────────
const ROI_DATA = [
  { cod: 'FS1000',        brand: 'Cummins',     roi: 83 },
  { cod: 'WF2073',        brand: 'Cummins',     roi: 83 },
  { cod: 'LF9009',        brand: 'Cummins',     roi: 83 },
  { cod: 'FF5488',        brand: 'Cummins',     roi: 83 },
  { cod: '3967726',       brand: 'Cummins',     roi: 65 },
  { cod: '0 281 006 064', brand: 'Bosch',       roi: 55 },
  { cod: '0 445 120 356', brand: 'Bosch',       roi: 65 },
  { cod: '0 445 120 455', brand: 'Bosch',       roi: 65 },
  { cod: '0 445 120 236', brand: 'Bosch',       roi: 65 },
  { cod: '0 445 120 217', brand: 'Bosch',       roi: 65 },
  { cod: '0 124 655 025', brand: 'Bosch',       roi: 50 },
  { cod: '0 445 010 537', brand: 'Bosch',       roi: 65 },
  { cod: '5265902',       brand: 'Delphi',      roi: 65 },
  { cod: '28231014',      brand: 'Delphi',      roi: 60 },
  { cod: 'BEBE4D24001',   brand: 'Delphi',      roi: 60 },
  { cod: 'BEBE4C17001',   brand: 'Delphi',      roi: 55 },
  { cod: '7C-3614',       brand: 'Caterpillar', roi: 40 },
  { cod: '20R-1278',      brand: 'Caterpillar', roi: 45 },
  { cod: '20R-1266',      brand: 'Caterpillar', roi: 45 },
];
...
if (ctxROI) {
  const brandColors = {
    'Cummins': 'rgba(0,212,255,.8)',
    'Bosch': 'rgba(0,180,216,.8)',
    'Delphi': 'rgba(255,140,0,.8)',
    'Caterpillar': 'rgba(201,168,76,.8)',
  };
  new Chart(ctxROI, {
    type: 'bar',
    data: {
      labels: ROI_DATA.map(d => d.cod),
      datasets: [{
        data: ROI_DATA.map(d => d.roi),
        backgroundColor: ROI_DATA.map(d => brandColors[d.brand]),
        borderRadius: 3,
        borderWidth: 0,
      }]
    },
    options: {
      ...
      plugins: {
        legend: { display: false },
        tooltip: { callbacks: { label: ctx => ' ' + ctx.raw + '% ROI' } }
      },
      scales: {
        x: { grid: { color: GRID }, ticks: { color: TICKS, callback: v => v + '%' }, max: 90 },
        y: { grid: { color: 'transparent' }, ticks: { color: TICKS, font: { size: 10 } } }
      }
    }
  });
...
// Bubble Chart (Cost × Price × Margin)
const ctxBUB = document.getElementById('chartBubble')?.getContext('2d');
if (ctxBUB) {
  const bubbleData = CATALOG.map(c => ({
    x: Math.round(c.price / 1.65),
    y: c.price,
    r: 4 + (c.stock === 'Stock' ? 6 : 4),
    label: c.code,
    brand: c.brand,
  }));
  const brandColors = {
    'Bosch': 'rgba(0,180,216,.7)',
    'Cummins': 'rgba(0,212,255,.7)',
    'Caterpillar': 'rgba(201,168,76,.7)',
    'Delphi': 'rgba(255,140,0,.7)',
  };
  new Chart(ctxBUB, {
    type: 'bubble',
    data: {
      datasets: bubbleData.map(b => ({
        ...
        backgroundColor: brandColors[b.brand] || 'rgba(0,212,255,.6)',
        borderColor: brandColors[b.brand]?.replace('.7', '.9') || '#00d4ff',
        borderWidth: 1,
      }))
    },
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: ctx => `Costo: €${ctx.raw.x} · Prezzo: €${ctx.raw.y}`
          }
        }
      },
      scales: {
        x: {
          grid: { color: GRID },
          ticks: { color: TICKS, callback: v => '€' + v },
          title: { display: true, text: 'Costo Acquisto (€)', color: TICKS, font: { size: 10 } },
          min: 0, max: 1200,
        },
        y: {
          grid: { color: GRID },
          ticks: { color: TICKS, callback: v => '€' + v },
          title: { display: true, text: 'Prezzo Vendita (€)', color: TICKS, font: { size: 10 } },
          min: 0, max: 1200,
        }
      }
    }
...
if (ctxCMG) {
  const qty = Array.from({length: 21}, (_, i) => i * 10);
  const cmg = qty.map(q => 330 + 0.36 * q);
  const rmg = qty.map(q => 606 - 0.95 * q);
  new Chart(ctxCMG, {
    type: 'line',
    data: {
      labels: qty,
    ...
    options: {
      ...
      scales: {
        x: { grid: { color: GRID }, ticks: { color: TICKS }, title: { display: true, text: 'Quantità (unità)', color: TICKS } },
        y: { grid: { color: GRID }, ticks: { color: TICKS, callback: v => '€' + v }, title: { display: true, text: 'Valore (€)', color: TICKS } }
      }
    }
  });
}
...
const ctxDS = document.getElementById('chartDS')?.getContext('2d');
if (ctxDS) {
  const qty2 = Array.from({length: 21}, (_, i) => i * 10);
  const demand = qty2.map(q => 800 - 0.24 * q);
  const supply = qty2.map(q => 300 + 0.1 * q);
  new Chart(ctxDS, {
    type: 'line',
    data: {
      labels: qty2,
      datasets: [
        ...
        },
        {
          ...
          data: supply,
          borderColor: '#ff8c00',
          backgroundColor: 'rgba(255,140,0,.05)',
          fill: false,
          tension: 0.2,
          pointRadius: 3,
    ...
    options: {
      ...
      scales: {
        x: { grid: { color: GRID }, ticks: { color: TICKS }, title: { display: true, text: 'Quantità (unità)', color: TICKS } },
        y: { grid: { color: GRID }, ticks: { color: TICKS, callback: v => '€' + v }, title: { display: true, text: 'Prezzo (€)', color: TICKS } }
      }
    }
  });
}
